...
<php>
    ...
    <script type="text/javascript">
		...
		function Search() {  //顯示所有討論
            db.collection("Opinion").get().then(function(querySnapshot) {   //查詢所有資料
            ...
            querySnapshot.forEach(function(doc) {
                ...
                doc.data()["內容"]+"<br /><br /><div align=\"right\"><a href=\"reply.php?id="+doc.data()["標題"]+"\" class=\"button\">Reply</a></div></td>"+"<td>"+doc.data()["投稿者"]+"</td></tr>";
      ...
      function storedata(){ //新增討論
				db.collection("Opinion").doc(title).set({
					標題:title,
					類型:style,
					內容:message,
					投稿者:name
      ...
      function keysearch(keyword) { //以keyword尋找討論
        db.collection("Opinion").get().then(function(querySnapshot) {   //查詢所有資料
          var flag=0;   //查詢指標，有查到為1，沒查到為0
          var str = "<table><thead align=\"center\"><tr><th>標題</th><th>類型</th><th>內容</th><th>投稿者</th></tr></thead><tbody>";
          querySnapshot.forEach(function(doc) {
              if (doc.data()["標題"].indexOf(keyword)!=-1){    //公司名稱不包含輸入值則顯示-1
              flag=1;
               str += "<tr><td>"+doc.data()["標題"]+"</td>"+"<td>"+doc.data()["類型"]+"</td>"+"<td>"+
               doc.data()["內容"]+"<br /><br /><div align=\"right\"><a href=\"reply.php?id="+doc.data()["標題"]+"\" class=\"button\">Reply</a></div></td>"+"<td>"+doc.data()["投稿者"]+"</td></tr>";
               document.getElementById("display").innerHTML=str;
      ...
      function keysearch2(keystyle) { //以類型尋找討論
        db.collection("Opinion").get().then(function(querySnapshot) {   //查詢所有資料
          var flag=0;   //查詢指標，有查到為1，沒查到為0
          var str = "<table><thead align=\"center\"><tr><th>標題</th><th>類型</th><th>內容</th><th>投稿者</th></tr></thead><tbody>";
          querySnapshot.forEach(function(doc) {
              if (doc.data()["類型"].indexOf(keystyle)!=-1){    //公司名稱不包含輸入值則顯示-1
              flag=1;
               str += "<tr><td>"+doc.data()["標題"]+"</td>"+"<td>"+doc.data()["類型"]+"</td>"+"<td>"+
               doc.data()["內容"]+"<br /><br /><div align=\"right\"><a href=\"reply.php?id="+doc.data()["標題"]+"\" class=\"button\">Reply</a></div></td>"+"<td>"+doc.data()["投稿者"]+"</td></tr>";
               document.getElementById("display").innerHTML=str;
				...
				<!-- Main -->
													...
													<form method="post" action="#">
														<div class="row gtr-uniform">
															<div class="col-4 col-12-xsmall">
																<input type="text" name="demo-keyword" id="keyword" value="" placeholder="keyword" />
															</div>
                              <div class="col-2 col-12-xsmall">
                                  <ul class="actions">
                                    <li><input type="button" value="Search" class="button primary icon solid fa-search" onclick="keysearch(keyword.value)"/></li>
                                  </ul>
															</div>
															<!-- Break -->
															<div class="col-4 col-12-xsmall">
																<select name="demo-keystyle" id="keystyle">
																	<option value="">- 選擇討論類型 -</option>
                                  <option value="帳號相關">帳號相關</option>
                                  <option value="BUG回報">BUG回報</option>
                                  <option value="遊戲問題">遊戲問題</option>
																	<option value="系統更新">系統更新</option>																	
																	<option value="連線登入">連線登入</option>
                                  <option value="執行安裝">執行安裝</option>
                                  <option value="玩家建議">玩家建議</option>
																</select>
															</div>
															<!-- Break -->
															<div class="col-2 col-12-xsmall">
                                  <ul class="actions">
                                    <li><input type="button" value="Search" class="button primary icon solid fa-search" onclick="keysearch2(keystyle.value)"/></li>
                                  </ul>
															</div>
														</div>
													</form><hr>
                    ...
                    <div class="row gtr-200">
                          ...
                          <h3>新增討論</h3>
													...
													<form method="post" action="#">
														<div class="fields row gtr-uniform">
															<div class="field half">
																<input type="text" name="demo-name" id="demo-name" value="" required placeholder="Name" />
															</div>
                              <div class="field half">
                                <input type="text" name="demo-title" id="demo-title" value="" required placeholder="Title" />
															</div>
                                  ...
                                  <option value="玩家建議">玩家建議</option>
															...
															<!-- Break -->
															<div class="field">
																<textarea name="demo-message" id="demo-message" placeholder="Enter your message" required rows="6"></textarea>
															</div>
															<!-- Break -->
															<div class="field"><div class="g-recaptcha" data-sitekey="6Le4AQcaAAAAAL1OdzW8mGzJqAXOYMZNEPL86c5_"></div></div>
															<div class="field">
                                  <ul class="actions">
                                    <li><input type="submit" value="Send Message" class="primary" onclick="check()" /></li>
                                    <li><input type="reset" value="Reset" /></li>
                                  </ul></div>
							</form>		
                          
						  <?php 
                            if (isset($_POST["demo-name"])&&isset($_POST["demo-title"])&&isset($_POST["demo-category"])&&isset($_POST["demo-message"])){
                              $name=$_POST["demo-name"];
                              $title=$_POST["demo-title"];
                              $style=$_POST["demo-category"];
